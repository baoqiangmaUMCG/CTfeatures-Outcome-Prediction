...
class ResEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = torch.cat((x1, x0_mp), 1)  # shape: (B, 64+64, 46, 45, 45)
        
        x1_mp = self.maxpool(x1) # shape: (B, 128, 23, 23, 23)
        x2 = torch.cat((x2, x1_mp), 1)  # shape: (B, 128+128, 23, 23, 23)
        
        x2_mp = self.maxpool(x2)  # shape: (B, 256, 12, 12, 12)
        x3 = torch.cat((x3, x2_mp), 1)  # shape: (B, 256+256, 12, 12, 12)
        
        x3_mp = self.maxpool(x3)  # shape: (B, 512, 6, 6, 6)
        x4 = torch.cat((x4, x3_mp), 1)  # shape: (B, 512+512, 6, 6, 6)
