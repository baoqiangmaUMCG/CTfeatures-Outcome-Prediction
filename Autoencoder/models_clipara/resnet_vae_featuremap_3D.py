...
class ResEncoder(nn.Module):
    def __init__(self,
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], shortcut_type, actfn=self.actfn)
        ...
        self.layer4_mu = self._make_layer(block, 512, layers[3], shortcut_type, stride=2, actfn=self.actfn, update_inplanes=False)
        self.layer4_var = self._make_layer(block, 512, layers[3], shortcut_type, stride=2, actfn=self.actfn)

        last_duration = int(math.ceil(sample_duration / 16))
        last_size = int(math.ceil(sample_size / 32))
        # self.avgpool = nn.AvgPool3d((last_duration, last_size, last_size), stride=1)

        # self.fc_mu = nn.Linear(512 * block.expansion, z_size)
        # self.fc_var = nn.Linear(512 * block.expansion, z_size)
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1, actfn=nn.ReLU(inplace=True), update_inplanes=True):
        downsample = None
    ...
    def forward(self, x):
        ...
        mu = self.layer4_mu(x)
        log_var = self.layer4_var(x)

        # mu = self.avgpool(mu)
        # log_var = self.avgpool(log_var)
...
class Res3DAutoencoder(nn.Module):
    ...
    def forward(self, x):
        mu, log_var = self.ResEncoder(x)
        z = self.reparameterize(mu, log_var)
        return self.ResDecoder(z), x, mu, log_var
