...
class ResEncoder(nn.Module):
    def __init__(self,
        ...
        self.conv1x1x1_4_to_bottom = conv1x1x1(in_planes=512, out_planes=256)
        self.conv1x1x1_3_to_bottom = conv1x1x1(in_planes=256, out_planes=128)
        self.conv1x1x1_2_to_bottom = conv1x1x1(in_planes=128, out_planes=64)
    ...
    def forward(self, x):
        ...
        # To bottom
        x4_up = self.conv1x1x1_4_to_bottom(x4)  # shape: (B, 256, 6, 6, 6)
        x4_up = self.Upsample3D_4(x4_up)  # shape: (B, 256, 12, 12, 12)
        x3 = x3 + x4_up  # shape: (B, 256, 12, 12, 12)
        
        x3_up = self.conv1x1x1_3_to_bottom(x3)  # shape: (B, 128, 12, 12, 12)
        x3_up = self.Upsample3D_3(x3_up)  # shape: (B, 128, 23, 23, 23)
        x2 = x2 + x3_up  # shape: (B, 128, 23, 23, 23)
        
        x2_up = self.conv1x1x1_2_to_bottom(x2)  # shape: (B, 64, 23, 23, 23)
        x2_up = self.Upsample3D_2(x2_up)  # shape: (B, 64, 46, 45, 45)
        x1 = x1 + x2_up  # shape: (B, 64, 46, 45, 45)
        
        x1_up = self.Upsample3D_1(x1)  # shape: (B, 64, 92, 90, 90)
        x0 = x0 + x1_up  # shape: (B, 64, 92, 90, 90)
        
        # To top
        x1 = x1 + x0_mp  # shape: (B, 64, 46, 45, 45)
        x1_mp = self.conv1x1x1_1_to_top(self.maxpool(x1))  # shape: (B, 128, 23, 23, 23)
        x2 = x2 + x1_mp  # shape: (B, 128, 23, 23, 23)
        
        x2_mp = self.conv1x1x1_2_to_top(self.maxpool(x2))  # shape: (B, 256, 12, 12, 12)
        x3 = x3 + x2_mp  # shape: (B, 256, 12, 12, 12)
        
        x3_mp = self.conv1x1x1_3_to_top(self.maxpool(x3))  # shape: (B, 512, 6, 6, 6)
        x4 = x4 + x3_mp  # shape: (B, 512, 6, 6, 6)
