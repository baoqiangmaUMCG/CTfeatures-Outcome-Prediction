...
def SeparableConv7x7x7(in_planes, out_planes):
    ...
    return nn.Sequential(nn.Conv3d(in_planes, 1, kernel_size=[1, 7, 7],
                         ...
                         nn.Conv3d(1, out_planes, kernel_size=[7, 1, 1],
                                   stride=[1,1,1], padding=[3, 0, 0], bias=False))
...
def SeparableTransConv7x7x7(in_planes, out_planes, output_pad=[0, 0, 0]):
    ...
    return nn.Sequential(nn.ConvTranspose3d(in_planes, 1, kernel_size=[1, 7, 7],
                         ...
                         nn.ConvTranspose3d(1, out_planes, kernel_size=[7, 1, 1],
                                            stride=[1,1,1], padding=[3, 0, 0], output_padding=[output_pad[0], 0, 0],
                                            bias=False))
...
class ResEncoder(nn.Module):
    def __init__(self,
        ...
        self.layer2 = self._make_layer(block, 32, layers[1], shortcut_type, stride=2, actfn=self.actfn)
        self.layer3 = self._make_layer(block, 32, layers[2], shortcut_type, stride=2, actfn=self.actfn)
        self.layer4 = self._make_layer(block, 32, layers[3], shortcut_type, stride=2, actfn=self.actfn)
        
        self.conv1x1x1_1_to_top = conv1x1x1(in_planes=32, out_planes=32)
        self.conv1x1x1_2_to_top = conv1x1x1(in_planes=32, out_planes=32)
        self.conv1x1x1_3_to_top = conv1x1x1(in_planes=32, out_planes=32)
    ...
    def forward(self, x):
        ...
        # To top
        x1 = x1 + x0_mp  # shape: (B, 64, 92, 90, 90)
...
class ResDecoder(nn.Module):
    def __init__(self,
        ...
        self.layerup4 = self._make_layer_upsample(
            block, 32, layers[3], shortcut_type, stride=2, actfn=self.actfn, output_pad=[1, 1, 1])
        self.layerup3 = self._make_layer_upsample(
            block, 32, layers[2], shortcut_type, stride=2, actfn=self.actfn, output_pad=[1, 1, 1])
        self.layerup2 = self._make_layer_upsample(
            block, 32, layers[1], shortcut_type, stride=2, actfn=self.actfn, output_pad=[1, 1, 1])
        ...
        self.conv1 = SeparableTransConv7x7x7(self.inplanes, output_channel, output_pad=[0, 0, 0])
...
class Res3DAutoencoder(nn.Module):
    ...
    def forward(self, x):
        x_latent,x_feature_map = self.ResEncoder(x)
