# IMPORTANT: OUT-OF-MEMORY ISSUES!
import torch
...
def SeparableConv7x7x7(in_planes, out_planes):
    ...
    return nn.Sequential(nn.Conv3d(in_planes, 1, kernel_size=[1, 7, 7],
                         ...
                         nn.Conv3d(1, out_planes, kernel_size=[7, 1, 1],
                                   stride=1, padding=[3, 0, 0], bias=False))
...
def SeparableTransConv7x7x7(in_planes, out_planes, output_pad=[0, 0, 0]):
    ...
    return nn.Sequential(nn.ConvTranspose3d(in_planes, 1, kernel_size=[1, 7, 7],
                         ...
                         nn.ConvTranspose3d(1, out_planes, kernel_size=[7, 1, 1],
                                            stride=1, padding=[3, 0, 0], output_padding=[output_pad[0], 0, 0],
                                            bias=False))
...
class ResEncoder(nn.Module):
    def __init__(self,
        ...
        self.Upsample3D_4 = nn.Upsample(size=(12, 12, 12), mode='trilinear', align_corners=False)
        self.Upsample3D_3 = nn.Upsample(size=(23, 23, 23), mode='trilinear', align_corners=False)
        self.Upsample3D_2 = nn.Upsample(size=(46, 45, 45), mode='trilinear', align_corners=False)
        self.Upsample3D_1 = nn.Upsample(size=(92, 90, 90), mode='trilinear', align_corners=False)
    ...
    def forward(self, x):
        ...
        # To bottom
        x4_up = self.Upsample3D_4(x4)  # shape: (B, 512, 12, 12, 12)
        x3 = torch.cat((x3, x4_up), 1)  # shape: (B, 256+512, 12, 12, 12)
        
        x3_up = self.Upsample3D_3(x3)  # shape: (B, 256+512, 23, 23, 23)
        x2 = torch.cat((x2, x3_up), 1)  # shape: (B, 128+256+512, 23, 23, 23)
        
        x2_up = self.Upsample3D_2(x2)  # shape: (B, 128+256+512, 46, 45, 45)
        x1 = torch.cat((x1, x2_up), 1)  # shape: (B, 64+128+256+512, 46, 45, 45)
        
        x1_up = self.Upsample3D_1(x1)  # shape: (B, 64+128+256+512, 92, 90, 90)
        x0 = torch.cat((x0, x1_up), 1)  # shape: (B, 64+64+128+256+512, 92, 90, 90)
        
        # To top
        x0_mp = self.maxpool(x0)  # shape: (B, 64+64+128+256+512, 46, 45, 45)
        x1 = torch.cat((x1, x0_mp), 1)  # shape: (B, 64+128+256+512 + 64+64+128+256+512, 46, 45, 45)
        
        x1_mp = self.maxpool(x1) # shape: (B, 64+128+256+512 + 64+64+128+256+512, 23, 23, 23)
        x2 = torch.cat((x2, x1_mp), 1)  # shape: (B, 128+256+512 + 64+128+256+512 + 64+64+128+256+512, 23, 23, 23)
        
        x2_mp = self.maxpool(x2)  # shape: (B, 128+256+512 + 64+128+256+512 + 64+64+128+256+512, 12, 12, 12)
        x3 = torch.cat((x3, x2_mp), 1)  # shape: (B, 256+512 + 128+256+512 + 64+128+256+512 + 64+64+128+256+512, 12, 12, 12)
        
        x3_mp = self.maxpool(x3)  # shape: (B, 256+512 + 128+256+512 + 64+128+256+512 + 64+64+128+256+512, 6, 6, 6)
        x4 = torch.cat((x4, x3_mp), 1)  # shape: (B, 512 + 256+512 + 128+256+512 + 64+128+256+512 + 64+64+128+256+512, 6, 6, 6)
...
class ResDecoder(nn.Module):
    def __init__(self,
                 ...
                 actfn,
                 shortcut_type='B',
                 input_channel=512 + 256+512 + 128+256+512 + 64+128+256+512 + 64+64+128+256+512):
        ...
        self.layerup4 = self._make_layer_upsample(
            block, 256, layers[3], shortcut_type, stride=2, actfn=self.actfn, output_pad=[1, 1, 1])
        self.layerup3 = self._make_layer_upsample(
            block, 128, layers[2], shortcut_type, stride=2, actfn=self.actfn)
        self.layerup2 = self._make_layer_upsample(
            block, 64, layers[1], shortcut_type, stride=2, actfn=self.actfn, output_pad=[1, 0, 0])
        ...
        self.conv1 = SeparableTransConv7x7x7(self.inplanes, 1, output_pad=[0, 1, 1])
